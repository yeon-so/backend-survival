...
* 도메인에는 명사에서 가져온 개념이 많이 있고, 이러한 객체들은 관계와 구조로 연결되어 있다. 그러나 무기력한 도메인 모델에서는 도메인 로직을 도메인 객체에 넣지 않는다는 디자인 규칙이 있다.&#x20;
* 도메인 로직은 도메인 객체 대신 도메인 객체에 대한 모든 연산을 담당하는 서비스 객체가 가지고 있다.
* 객체의 상태와 행동을 함께 갖고 있는 객체지향과는 거리가 있어 사용 시 주의가 필요하다.
